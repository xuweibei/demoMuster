...
import { getUniversityByBranchId, addStudentList, addActivity, updateActivity, deleteById,addActivityAndStudent } from '@/actions/base';
...
class AddStudentManage extends React.Component {
    ...
    componentWillMount() {
        this.loadBizDictionary(['dic_Status', 'dic_YesNo', 'activity_type','reg_source','is_study']);
    ...
    fetch = (params = {}) => {
        ...
        this.props.addStudentList(condition).payload.promise.then((response) => {
            let data = response.payload.data;
            if (data.state === 'success') {
                var list = []
                data.data = data.data || []
                data.data.map(item => {
                    item.key = item.studentId;
                    list.push(item);
                })
                this.setState({
                    data_list: list,
    ...
    onBatchAdd = () => {
        //获取学生ids数组
        var studentIdslist = [];
        this.state.UserSelecteds.map(studentId => {
            studentIdslist.push(studentId)
        })

        this.props.addActivityAndStudent({ studentIds: studentIdslist.join(","),activityId:this.state.pagingSearch.activityId }).payload.promise.then((response) => {
            let data = response.payload.data;
                            ...
                            <div className="search-paging">
                                ...
                                <Col span={12}>
                                        <div className='flex-row'>
                                            {this.state.UserSelecteds.length > 0 ? <Button onClick={this.onBatchAdd} icon="plus">确定</Button> : <Button disabled icon="plus">确定</Button>}
                                        </div>
                                    </Col>
...
//表单组件 封装
const WrappedManage = Form.create()(AddStudentManage);
...
function mapDispatchToProps(dispatch) {
    return {
        ...
        addStudentList: bindActionCreators(addStudentList, dispatch),//活动管理-学生-新增学生-列表查询接口
        addActivityAndStudent:bindActionCreators(addActivityAndStudent, dispatch),//活动管理-学生-新增学生-确定按钮
