...
class ProductPriceApplyView extends React.Component {
    constructor(props) {
        ...
        this.state = {
            ...
            dic_fenqi: [1, 2, 3, 4, 5, 6, 7, 8].map((item, index) => {
        ...
        if (this.state.dataModel.productType == 1) {
            this.props.recruitBindProductPriceInfoUpById(productPriceInfo.productId, productPriceInfo.recruitBatchId).payload.promise.then((response) => {
                ...
                if (data.result === false) {
                    ...
                    totalCoursePrice = Math.round(totalCoursePrice*100)/100;
    ...
    calcTotalStandardPrice = (auto) => {
        ...
        if (auto) {
            this.state.product_price_list.map((item, index) => {
                // if (index + 1 < this.state.product_price_list.length) {
                    item.coursePrice = Math.round((item._coursePrice * 1.00) / _totalCoursePrice * totalCoursePrice*100)/100;
                    lastCoursePrice += parseFloat(item.coursePrice);
                    lastCoursePrice = Math.round(lastCoursePrice*100)/100;
                // }
                // else {
                //     item.coursePrice = Math.round((totalCoursePrice - lastCoursePrice)*100)/100;//剩余金额
                // }
            })
        }
        else {
            this.state.product_price_list.map((item, index) => {
                // if (index + 1 < this.state.product_price_list.length) {
                    ...
                    lastCoursePrice += Math.round(item.coursePrice*100)/100;
                // }
                // else {
                //     item.coursePrice = Math.round((totalCoursePrice - lastCoursePrice)*100)/100;//剩余金额
                // }
        ...
        //最后一项金额计算
        for(let i=this.state.product_price_list.length-1;i>=0;i--){
            if(this.state.product_price_list[i].coursePrice != 0){
                let result = (parseFloat(this.state.product_price_list[i].coursePrice) + parseFloat(totalCoursePrice - lastCoursePrice)).toFixed(2);
                if(result >= 0){
                    this.state.product_price_list[i].coursePrice = result;
                    lastCoursePrice = totalCoursePrice;
                    break;
                }
            }
        }
        ...
        this.state.dic_fenqi
            ...
            .map((item, index) => {
                if (item.id < this.state.currentFenQi) {
                    ...
                    lastCoursePrice += Math.round(item.money*100)/100;
                }
                else {
                    item.money = Math.round((totalCoursePrice - lastCoursePrice)*100)/100;//剩余金额
                }
            })
        this.setState({ dic_fenqi: this.state.dic_fenqi })
    }
                                    ...
                                    {getFieldDecorator('productTotalPriceApply', {
                                        ...
                                        onChange={(value) => {
                                            if (isNaN(value) || value == '') {
                                                ...
                                                this.state.dataModel.productTotalPriceApply = Math.round(value*100)/100;
                                            }
                                            //同步价格更新
