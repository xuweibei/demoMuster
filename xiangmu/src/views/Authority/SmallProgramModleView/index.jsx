...
import { SmallProgramModleView, SmallProgramModleEdit,SmallProgramMenuList } from '@/actions/admin';
...
class HomeModleView extends React.Component {
    ...
    loadOrgTypeAllFuntions = () => {
        ...
        this.props.SmallProgramMenuList(params).payload.promise.then((response) => {
    ...
    //载入角色功能清单
    loadRoleFuntions = () => {
        this.props.SmallProgramModleView({roleId:this.state.dataModel.roleId,moduleType: this.state.dataModel.orgType}).payload.promise.then((response) => {
            let data = response.payload.data; 
            if (data.state != 'success') {
                message.error(data.message, 3);
            }
    ...
    onSubmit = () => {
        ...
        this.props.form.validateFields((err, values) => {
            ...
            if (!err) {  
                ...
                this.props.SmallProgramModleEdit({roleId:this.state.dataModel.roleId,moduleIds: this.state.funids.join(',')}).payload.promise.then((response) => {
                    let data = response.payload.data; 
                    if (data.state == 'success') {
    ...
    renderTree(menus, isPower) { 
        ...
        return menus.map((item) => {
            ...
            return <TreeNode title={title} key={item.moduleCode.toString()} dataItem={item} isLeaf={true}>{childs}</TreeNode>
        })
    }
    //表单按钮处理
    renderBtnControl() {
...
function mapDispatchToProps(dispatch) {
    return {
        SmallProgramModleView: bindActionCreators(SmallProgramModleView, dispatch),
        SmallProgramModleEdit: bindActionCreators(SmallProgramModleEdit, dispatch),
