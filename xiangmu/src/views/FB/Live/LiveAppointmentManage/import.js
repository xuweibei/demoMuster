...
class LiveStudentImport extends React.Component {
  constructor(props){
    super(props);
    this.state= {
      editMode: '',
  ...
  onSave = () => {
    ...
    formData.append('liveId', this.state.pagingSearch.liveId);
    formData.append('courseArrangeId', this.state.pagingSearch.courseArrangeId);
      ...
      if (data.state == 'success') {
        ...
        if(typeof data.data == 'number'){
          that.setState({
            ...
            errorCount: 0,
            error_url: `/edu/live/exportErrorMsg`,
          })
        }else{
          that.setState({
            isImported: true,
    ...
    let uploadUrl = `${env.serverURL}/edu/live/importBookStudents?token=${token}`;
  ...
  render(){
              ...
              <Row gutter={24}>
                <Col span={24}>
                  <FormItem {...searchFormItemLayout} label="直播名称">
                    {this.state.pagingSearch.liveName}
                  </FormItem>
                </Col>
                <Col span={24}>
                  <FormItem {...searchFormItemLayout} label="课程班名称">
                    {this.state.pagingSearch.coursePlanName}
                  </FormItem>
                </Col>
                <Col span={24}>
                  <FormItem {...formItemLayout24} label="下载">点击&nbsp;
                    {/*<a onClick={() => { window.open('') }}>下载上传模板</a>*/}
                    <FileDownloader
                      apiUrl={'/edu/live/exportBookTemplate'}//api下载地址
                      method={'post'}//提交方式
                      options={{ }}
                      title={'下载上传模板'}
                    >
                ...
                <Col span={24}>
                  <FormItem {...formItemLayout24} label='选择上传文档'>
                    ...
                    <Icon style={{color:'#e89abe',paddingRight:'5px'}} type="exclamation-circle" />已预约过其他直播的学员不可再次预约
                  </FormItem>
                </Col>
                {
    ...
    return (
      <ContentBox titleName={"学员导入"} bottomButton={
        <FormItem
...
//表单组件 封装
const WrappedManage = Form.create()(LiveStudentImport);
