...
import { NewleaveRefundAuditQueryList, getCourseStudentRefundCheckStudentChange, courseStudentRefundChangeAudit } from '@/actions/course';
...
class LeaveRefundAudit extends React.Component {
    ...
    componentWillMount() {
        //载入需要的字典项: 审核状态
        this.loadBizDictionary(['student_change_refund_status_front','confirm_status']);
        //首次进入搜索，加载服务端字典项内容
        this.onSearch();
    }
    ...
    },
    {
        ...
        render: (text, record) => {
            return <DropDownButton>
                {record.statusNum != 9 && <Button onClick={() => {
                    this.onLookView('View', record)
                }}>查看</Button>
                }
                {record.statusNum == 9 && <Button onClick={() => {
                    this.props.getCourseStudentRefundCheckStudentChange({ studentChangeId: record.studentChangeId,auditOrgType:5 }).payload.promise.then((response) => {
    ...
    fetch(params) {
        ...
        // condition.isBranch = 2;//分部操作
        condition.auditOrgType = 5;
                                            ...
                                            <FormItem {...searchFormItemLayout} label="审核状态">
                                                {getFieldDecorator('status', {
                                                    ...
                                                    <Select >
                                                        ...
                                                        {this.state.student_change_refund_status_front.filter(item=> item.value!=1 && item.value!=2 && item.value!=3 && item.value!=4 && item.value!=5 && item.value!=6 && item.value!=7 && item.value!=8 ).map((item, index) => { 
