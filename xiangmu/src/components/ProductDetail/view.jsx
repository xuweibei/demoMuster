...
import { productPriceGetById } from '@/actions/recruit';
...
class ProductView extends React.Component {
    ...
    componentWillMount() {
        ...
        if (this.state.dataModel.productType == 1) {
            ...
            //     }
            // })
            this.props.productPriceGetById({productPriceId:this.state.dataModel.productPriceId}).payload.promise.then((response) => {
                let data = response.payload.data;
                if (data.result === false) {
                    this.setState({ loading: false })
                    message.error(data.message);
                    ...
                    this.setState({
                        loading: false,
                        dataModel: product,
                        data: data.data.productCoursePriceLst || [],
                    })
                }
            })
        }
                        ...
                        columns = [
                            ...
                            },
                            {
                                ...
                                render: (text, record, index) => {
                                    return <span>{this.state.dataModel.classTypeName}</span>;
                                }
                            },
                            {
                                title: '授课方式',
...
function mapDispatchToProps(dispatch) {
    return {
        ...
        productPriceGetById: bindActionCreators(productPriceGetById, dispatch),
    };
}
