...
//业务接口方法引入
import { getTeacheringByUser, deleteTeacheringByUser } from '@/actions/base';
//业务数据视图（增、删、改、查)
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            pagingSearch: {
                currentPage: 1,
                pageSize: 10,
                userId: props.currentDataModel.userIds,
            },
            data: [],
            totalRecord: 0,
            loading: false,
...
　　　　　　 width:280,
        ...
        },
        {
            ...
            width: 120,
            render: (text, record) => (
                <Button onClick={() => { this.onDeltet(record.userScopeId) }}>删除</Button>
            ),
        }
    ...
    onAdd = () => {
        this.props.viewCallback({ userIds: [this.state.dataModel.userIds], dataModel: this.state.dataModel, });//合并保存数据
    }
    onCancel = () => {
        this.props.viewCallback({ userIds: '', universityIds: '', cancel: true });//合并保存数据
    }
    onDeltet = (universityId) => {
        let params = { userScopeId: universityId }
        this.props.deleteTeacheringByUser(params).payload.promise.then((response) => {
    ...
    fetch = (params = {}) => {
        ...
        this.props.getTeacheringByUser(condition).payload.promise.then((response) => {
            let data = response.payload.data;  
            if(data.state == 'success'){
                this.setState({ pagingSearch: condition, data: data.data, loading: false })
            }else{
            ...
            <div className="search-result-list">
                ...
                <div className="search-paging">
                    <Row justify="center" align="middle" type="flex">
                        <Button onClick={this.onAdd} icon="plus">新增负责教学点</Button>
...
function mapDispatchToProps(dispatch) {
    return {
        ...
        //各业务接口
        getTeacheringByUser: bindActionCreators(getTeacheringByUser, dispatch),
        deleteTeacheringByUser: bindActionCreators(deleteTeacheringByUser, dispatch),
