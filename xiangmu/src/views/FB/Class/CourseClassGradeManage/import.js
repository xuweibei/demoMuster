...
class GradeImport extends React.Component {
  ...
  onSave = () => {
      ...
      if (data.state == 'success') {
        ...
        if(typeof data.data == 'number'){
          that.setState({
            ...
            errorCount: 0,
            error_url: `/edu/studentScore/exportErrorStudentScoreVos`,
          })
        }else{
          that.setState({
            isImported: true,
    ...
    let uploadUrl = `${env.serverURL}/edu/studentScore/importStudentScoreVos?token=${token}`;
  ...
  render(){
              ...
              <Row gutter={24}>
                <Col span={12}>
                  <FormItem {...searchFormItemLayout} label="开课批次">
                    {this.state.pagingSearch.courseplanBatchName}
                  </FormItem>
                </Col>
                <Col span={12}>
                  <FormItem {...searchFormItemLayout} label="科目">
                    {this.state.pagingSearch.courseCategoryName}
                  </FormItem>
                </Col>
                <Col span={12}>
                ...
                <Col span={12}>
                  <FormItem {...searchFormItemLayout} label="课程班类型">
                    {this.state.pagingSearch.stateMsg}
                  </FormItem>
                </Col>
                <Col span={24}>
                  <FormItem {...searchFormItemLayout24} label="下载">点击&nbsp;
                    ...
                    <FileDownloader
                      apiUrl={'/edu/studentScore/exportStudentScoreVoTemplate'}//api下载地址
                      method={'post'}//提交方式
                      options={{ }}
                      title={'下载上传模板'}
                    >
                    </FileDownloader>
                    &nbsp;&nbsp;整理课程班 学生报考及成绩信息，学生报考信息以最后导入为准。
                  </FormItem>
                </Col>
    ...
    return (
      <ContentBox titleName={"课程班学生成绩导入"} bottomButton={
        <FormItem
