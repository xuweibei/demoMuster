...
import { queryByActivityName } from '@/actions/recruit';
        ...
        if (value) {
            if (re.test(value)) //使用正则判断是否存在中文
            {
                if (value.match(re).length < 3) { //返回中文的个数
                    ...
                    this.props.queryByActivityName(value).payload.promise.then((response) => {
                        ...
                        if (data.state === 'success') {
                            this.setState({
                                dataSource: data.data.map((a) => {
                                    ...
                                    return { id: `${a.activityId}`, name: `${a.activityName}` }
...
function mapDispatchToProps(dispatch) {
    return {
        ...
        queryByActivityName: bindActionCreators(queryByActivityName, dispatch),
    };
}
