...
import { getCourseplanList, updateCoursePlan, updateIspilot } from '@/actions/course';
...
class CourseplanModify extends React.Component {
    ...
    }, 
    {
      ...
      render: (text, record) => {
        return <DropDownButton>
          ...
          {!record.isPilot && <Button onClick={() => { this.setPilot(record); }}>教服试点</Button>}
          {record.isPilot && <Button onClick={() => { this.setPilot(record); }}>取消试点</Button>}
        </DropDownButton>
      }
    }
  ];
  ...
  fetch(params) {
    ...
    condition.isUpatePage = 1;//是否是开课计划修改页面
  ...
  setPilot = (data) => {
    Modal.confirm({
        title: data.isPilot ? '确定取消教服试点吗?' : '确定设为教服试点吗?',
        content: data.isPilot ? '点击确认取消教服试点!否则点击取消！': '点击确认设为教服试点!否则点击取消！',
        onOk: () => {
            var params = {
              courseplanId: data.courseplanId,
              isPilot: data.isPilot ? 0 : 1
            }
            this.props.updateIspilot(params).payload.promise.then((response) => {
                let data = response.payload.data;
...
//表单组件 封装
const WrappedManage = Form.create()(CourseplanModify);
...
function mapDispatchToProps(dispatch) {
  return {
    ...
    updateIspilot: bindActionCreators(updateIspilot, dispatch),
