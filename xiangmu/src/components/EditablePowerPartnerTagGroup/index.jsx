...
import { searchUserPartnerScope } from '@/actions/partner';
        ...
        if (value) {
            if (re.test(value)) //使用正则判断是否存在中文
            {
                if (value.match(re).length < 3) { //返回中文的个数
                    ...
                    this.props.searchUserPartnerScope(value).payload.promise.then((response) => {
                        ...
                        if (data.state === 'success') {
                            this.setState({
                                dataSource: data.data.map((a) => {
                                    ...
                                    return { id: `${a.partnerId}`, name: `${a.partnerName}`,branchName:`${a.branchName}`,partnerClassType:`${a.partnerClassType}` }
