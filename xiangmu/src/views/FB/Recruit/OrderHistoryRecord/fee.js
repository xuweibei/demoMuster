...
class FeeEdit extends React.Component {
    ...
    onGetFeeQrCode(){
      ...
      var params = [{
        ...
        totalMoney: this.state.pos_info.totalAmount,
    ...
    onSubmit = (values) => {
      ...
      this.props.form.validateFields((err, values) => {
        if(!err){
          ...
          var params = {
            ...
            otherPayNo: "AABBAA",   //暂时随便填写
            money: that.state.dataModel.currentPayableAmount,
            posCode: "1",   //暂时写死 北京分部 对应的pos机编号
            feeClass: 1,
            remark: values.applyReason
          }
          that.props.paymentByPos(params).payload.promise.then((response) => {
              let data = response.payload.data;
              if (data.state === 'error') {
                  message.error(data.message);
              }
      ...
      //that.props.viewCallback({ ...values, orderId: this.props.orderId });
    }
    onShowQrCode() {
      this.setState({
        isShowQrCodeModal: true
    ...
    render() {
            ...
            <Col span={24}>
              <div className='block_content' style={{margin:'0 auto'}}>
                <span style={{color:"red"}}>
                  第1期欠缴金额（¥） ：{formatMoney(this.state.dataModel.currentPayableAmount)}          收费方：{this.state.dataModel.payeeTypeStr}
                </span>
              </div>
            </Col>
          </Row>
      ...
      return (
        <ContentBox titleName={"订单缴费"} bottomButton={
          <FormItem className='btnControl' {...btnformItemLayout}>
            <Button onClick={this.onSubmit} icon="save" type="primary">缴费</Button>
