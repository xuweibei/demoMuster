...
class StudentPayfeeConfirmManage extends React.Component {
    ...
    onChangeDate=()=>{
        this.props.form.validateFields((err, values) => {
            ...
            this.props.getStudentPayfeeConfirmBatchConfirm({ studentPayfeeIds: this.state.UserSelecteds.join(','),confirmDate:formatMoment(confir) }).payload.promise.then((response) => {
                let data = response.payload.data;
                if (data.result === false) {
                    message.error(data.message);
                }
