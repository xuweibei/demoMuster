    ...
    },
    {
        ...
        render: (text, record) => {
            return <DropDownButton>
                {record.statusNum != 1 && <Button onClick={() => {
                    this.onLookView('View', record)
                }}>查看</Button>
                }
                {record.statusNum == 1 && <Button onClick={() => {
                    this.props.getCourseStudentRefundCheckStudentChange({ studentChangeId: record.studentChangeId,auditOrgType:1 }).payload.promise.then((response) => {
