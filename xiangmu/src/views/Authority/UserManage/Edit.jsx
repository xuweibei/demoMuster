...
import { Modal, Form, Row, Col, Input, Select, Button, Icon, Table, Pagination, Tree, message, Checkbox } from 'antd';
...
import { editBatchAdminUser } from '@/actions/base';
    ...
    onSubmit = () => {
        ...
        this.props.form.validateFields((err, values) => {
            if (!err) {
               ...
               console.log(this.state.dataModel.userIds.join(','),values.itemIds)
               let condition = {};
               condition.userIds = this.state.dataModel.userIds.join(',');
               condition.itemIds = Array.isArray(values.itemIds)?values.itemIds.join(','):values.itemIds;
               this.props.editBatchAdminUser(condition).payload.promise.then((response) => {
                                ...
                                >  
                                  {/* 查找批量选择的人的姓名 */}
                                    {this.state.dataModel.userIds.map((item) => {
                                        let userInfo = this.state.dataModel.data.find(a => a.userId == item);
                            ...
                            <Col span={24}>
                                ...
                                >
                                    {getFieldDecorator('itemIds', {
                                        initialValue: this.state.dataModel.itemList.map(a => a.toString()),
                                    })(
