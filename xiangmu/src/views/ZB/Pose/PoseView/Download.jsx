...
class Download extends React.Component {
  ...
  onExport = () => {
    ...
    var url = `${env.serverURL}/edu/pos/importPos?token=${env.getToken()}`;
  ...
  onSave = () => {
    ...
    const successFn = (response) => {
      ...
      if (data.state == 'success') {
        ...
        that.setState({
          ...
          error_url: `/edu/pos/exportErrPos`,
        })
    ...
    let uploadUrl = `${env.serverURL}/edu/pos/importPos?token=${token}`;
    const xhr = new XMLHttpRequest
  ...
  render() {
              ...
              <Row type='flex' justify='center'>
                <div style={{marginBottom:20}}>
                  <FileDownloader
                    ...
                    method={'post'}//提交方式
                    options={{ filePath: 'upload/importTemplate/uploadPos.xlsx' }}
                    title={'下载模板'}
                  >
                  </FileDownloader>
                </div>
    ...
    return (
      <ContentBox titleName={"导入Pose机"} bottomButton={
        <FormItem
...
//表单组件 封装
const WrappedManage = Form.create()(Download);
