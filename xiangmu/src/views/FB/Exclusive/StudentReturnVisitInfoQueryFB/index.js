...
import { returnVisitStudentSelectSivitInFb } from '@/actions/stuService';

class StudentReturnVisitInfoQueryFB extends React.Component {
  constructor(props) {
    super(props);
    //扩展方法用于本组件实例
    ...
    if(parseInt(startTimeMonth) - 6 > 0){
      startTimeMonth = parseInt(startTimeMonth) - 6;
    }else if(parseInt(startTimeMonth) - 6 == 0){
      startTimeMonth = 12;
      startTimeYear = parseInt(startTimeYear) - 1;
    }else{
      startTimeMonth = 12 - Math.abs(parseInt(startTimeMonth) - 6);
      startTimeYear = parseInt(startTimeYear) - 1;
    }
  ...
  fetch(params = {}) {
    ...
    this.props.returnVisitStudentSelectSivitInFb(condition).payload.promise.then((response) => {
    ...
    switch (this.state.editMode) {
      ...
      default:
        ...
        const prefixSelector = getFieldDecorator('textKey', {
          ...
          <Select style={{ width: 80 }} onChange={this.onCountryChange}>
...
//表单组件 封装
const WrappedManage = Form.create()(StudentReturnVisitInfoQueryFB);
...
function mapDispatchToProps(dispatch) {
  return {
    ...
    returnVisitStudentSelectSivitInFb: bindActionCreators(returnVisitStudentSelectSivitInFb, dispatch),
