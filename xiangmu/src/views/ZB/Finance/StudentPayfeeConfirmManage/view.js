...
class StudentPayfeeConfirmManageView extends React.Component {
    constructor(props) {
        super(props)
    ...
    onSubmit = () => {
        ...
        this.props.form.validateFields((err, values) => {
            if(!err){
                ...
                if(formatMoment(confir) < timestampToTime(this.state.dataModel.payDate)){
                    message.warning('财务确认日期不能小于缴费日期!');
                    return;
                }
                ...
                if(this.props.editMode == 'Confirm'){
                    this.props.getStudentPayfeeConfirmBatchConfirm({ studentPayfeeIds: this.props.currentDataModel.studentPayfeeId,confirmDate:formatMoment(confir),financeStatus: values.financeStatus || 2 }).payload.promise.then((response) => {
                        let data = response.payload.data;
                        ...
                        <Row gutter={24}>
                            ...
                            </Col>
                            {
                                ...
                                <div>
                                    ...
                                    <Col span={12}>
                                        <FormItem {...searchFormItemLayout} label="支付单号">
                                            {this.state.dataModel.payCode}
                                        </FormItem>
                                    </Col>
                                </div>
                                : ''
                            ...
                            {
                                (this.props.editMode=='Confirm' && this.state.dataModel.paymentWay == 3) ? <Col span={12}>
                                <FormItem {...searchFormItemLayout} label="财务确认情况">
                                {getFieldDecorator('financeStatus', {
                                        initialValue: '',
                                        rules: [{
                                            required: true, message: '请选择财务确认情况!',
                                        }],
                                    })(
                                        <Select >
                                            <Option value="">--请选择--</Option>
                                            {this.props.finance_status.filter(A => A.value != 1).map((item, index) => {
                                ...
                                : <Col span={24}>
                                    <FormItem {...searchFormItemLayout24} label="财务确认情况">
                                        {getDictionaryTitle(this.props.finance_status, this.state.dataModel.financeStatus)}
                                    </FormItem>
                                </Col>
                            }
                            <Col span={24}></Col>
...
//表单组件 封装
const WrappedManage = Form.create()(StudentPayfeeConfirmManageView);
