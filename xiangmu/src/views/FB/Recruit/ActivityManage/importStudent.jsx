...
class ImportStudent extends React.Component {
  ...
  onExport = () => {
    ...
    var url = `${env.serverURL}/edu/activityStudentImport/import?token=${env.getToken()}`;
  ...
  onSave = () => {
    ...
    formData.append('activityId',this.props.currentDataModel.activityId);
    ...
    const successFn = (response) => {
      ...
      if (data.state == 'success') {
        ...
        that.setState({
          ...
          oldCount: data.data.oldCount,
          errorCount: data.data.errorCount,
          errorJsonObj: data.data.error,
          error_url: `/edu/activityStudentImport/exportError`,
        })
        //param.success({
    ...
    let uploadUrl = `${env.serverURL}/edu/activityStudentImport/import?token=${token}`;
    const xhr = new XMLHttpRequest
  ...
  render() {
              ...
              <Row gutter={24}>
                ...
                <Col span={24}>
                  ...
                  <FileDownloader
                    ...
                    method={'post'}//提交方式
                    options={{ filePath: 'upload/importTemplate/studentActivityImport.xlsx' }}
                    title={'下载模板'}
                  >
                  </FileDownloader>
                  </div>
                ...
                </Col>
                {
                  ...
                  <Col span={24}>
                    成功导入{this.state.successCount}条(新生{this.state.newCount}个,老生{this.state.oldCount}个)。导入失败{this.state.errorCount}条。
    ...
    return (
      <ContentBox titleName={"上传活动参与学生名单"} bottomButton={
        <FormItem
          className='btnControl'
          {...btnformItemLayout}
        >
          <Button type="primary" loading={this.state.uploading} icon="save" onClick={this.onSave}>上传</Button>
...
//表单组件 封装
const WrappedManage = Form.create()(ImportStudent);
