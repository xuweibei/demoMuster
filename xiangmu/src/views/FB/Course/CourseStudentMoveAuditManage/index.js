...
import { getCourseFBStudentMoveAuditList, getFBCheckStudentChange, courseFBStudentMoveAudit } from '@/actions/course';
...
class CourseStudentMoveAuditManage extends React.Component {
    ...
    },
    {
        ...
        render: (text, record) => {
            return <DropDownButton>
                ...
                {record.status == 1 && <Button onClick={() => {
                    this.props.getFBCheckStudentChange({ studentChangeId: record.studentChangeId }).payload.promise.then((response) => {
    ...
    fetch(params) {
        ...
        this.props.getCourseFBStudentMoveAuditList(condition).payload.promise.then((response) => {
            let data = response.payload.data;
    ...
    onViewCallback = (dataModel) => {
        if (!dataModel) {
            ...
            switch (this.state.editMode) {
                case "Audit": //提交
                    this.props.courseFBStudentMoveAudit(dataModel).payload.promise.then((response) => {
                        let data = response.payload.data;
                        if (data.state === 'error') {
                            message.error(data.message);
                        }
                                        ...
                                        <Col span={12}>
                                            <FormItem {...searchFormItemLayout} label="转出订单学生姓名">
                                                {getFieldDecorator('studentName', {
                                                    ...
                                                    <Input placeholder='转出订单学生姓名'/>
                                                    )}
                                            </FormItem>
                                        </Col>
                                        <Col span={12}>
                                            <FormItem {...searchFormItemLayout} label="审核人">
...
function mapDispatchToProps(dispatch) {
    return {
        ...
        getCourseFBStudentMoveAuditList: bindActionCreators(getCourseFBStudentMoveAuditList, dispatch),
        getFBCheckStudentChange: bindActionCreators(getFBCheckStudentChange, dispatch),
        courseFBStudentMoveAudit: bindActionCreators(courseFBStudentMoveAudit, dispatch),
    };
}
//redux 组件 封装
