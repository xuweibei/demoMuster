...
import { DelayManagementList,DeferredAuditBatchApply,OutGoingTaskBatcNohArrive,DeferredAuditApply } from '@/actions/base'
...
import StudentAskfaceView from './BatchAudit';
    ...
    },
    {
      ...
      render: (text, record) => {
        return <DropDownButton>
          {
            (record.auditStatus==1&&record.isFinalAudit==0)?<Button onClick={() => { this.onLookView('ViewEdit', record); }}>审核</Button>:
  ...
  onViewCallback = (dataModel) => {
    if (!dataModel) {
      ...
      switch (this.state.editMode) {
        case 'editpUser':
          this.props.DeferredAuditBatchApply(dataModel).payload.promise.then((response) => {
            let data = response.payload.data;
            if (data.state === 'error') {
              message.error(data.message);
            }
          ...
          break;
          case 'ViewEdit':
          this.props.DeferredAuditApply(dataModel).payload.promise.then((res)=>{
        ...
        var rowSelection = {
          ...
          getCheckboxProps: record => {
            ...
            return ({
              ...
              disabled: (record.auditStatus==1&&record.isFinalAudit==0)? false:true,    // Column configuration not to be checked
            })
          },
        };
...
function mapDispatchToProps(dispatch) {
  return {
    ...
    DeferredAuditBatchApply: bindActionCreators(DeferredAuditBatchApply, dispatch),//批量审核
    DeferredAuditApply: bindActionCreators(DeferredAuditApply, dispatch),//审核
  };
}
//redux 组件 封装
