...
//数据转字典标题
import { getAllTeacherList } from '@/actions/teaching';

class EditableTeacherMain extends React.Component {
    input = null
    constructor(props) {
        super(props)
        //扩展方法用于本组件实例
    ...
    renderOption(item) {
        return <Option key={item.id} text={item.name}>
                  {item.name}
                  <span style={{float:'right'}}>{item.englishName ? item.englishName : ''}</span>
                </Option>
    }
    handleSearch = (value) => {
        ...
        if (value && value.indexOf(' ')!=0) {
            ...
            this.props.getAllTeacherList({realName:value,currentPage:1,pageSize:100}).payload.promise.then((response) => {
                ...
                if (data.state === 'success') {
                    this.setState({
                        dataSource: data.data.map((a) => {
                            ...
                            return { id: `${a.teacherId}`, name: `${a.realName}`, englishName:a.englishName }
...
function mapDispatchToProps(dispatch) {
    return {
        ...
        getAllTeacherList: bindActionCreators(getAllTeacherList, dispatch),
    };
}
export default connect(mapStateToProps, mapDispatchToProps)(EditableTeacherMain);
