...
import { queryMaxDiscountQuotaByItems, queryPayeeTypeByBranchId, checkCourseIsStudy, getCourseCategoryPrice, postRecruitBatchPageList } from '@/actions/recruit';

import ProductView from '@/components/ProductDetail/view'

const searchFormItemLayout16 = {
  labelCol: { span: 8 },
  wrapperCol: { span: 16 },
};
//let STAFF_LIMIT = 200;
...
class OrderEdit extends React.Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      term_num: 1,    //默认一期，即整单
      courseCategoryList: [],
      createDate: timestampToTime(new Date(2018,7,1,0,0,0).getTime(), true),
  ...
  getRecruitList() {
    ...
    this.props.postRecruitBatchPageList(params).payload.promise.then((response) => {
      let data = response.payload.data;
      if (data.state === 'success') {
        data.data = data.data.filter(A => A.state == 1);
        this.setState({ recruit_data_list: data.data })
      }
      else {
        message.error(data.message);
      }
      ...
      this.props.getOrderById(this.props.orderId).payload.promise.then((response) => {
        ...
        if (data.state === 'success') {
          ...
          that.setState({
            ...
            createDate: data.data.createDateStr
          });
    ...
    if (this.state.editMode == "OrderClassAmateur" || this.state.editMode == "OrderPerson" || this.state.editMode == "OrderPersonOnline") {
      if (this.state.term_list && this.state.term_list.length) {
        ...
        if (shouldUpdate) {
          ...
          this.state.term_list[this.state.term_list.length - 1].payableAmount = (totalAmount - _tempAmount).toFixed(2);
        }
      }
      setTimeout(function () {
  ...
  onMoneyNumberChange(value, term) {
    ...
    this.state.product_list.map(item => {
      var _money2 = 0;
      if(item.discount && item.discount.productDiscountPrice){
        _money2 = parseFloat(item.productTotalPrice || 0) * parseFloat(item.discount.productDiscountPrice || 100) / 100 * 10000;
      }else{
        _money2 = parseFloat(item.productTotalPrice || 0) * 10000;
      }
      amount += parseFloat(_money2 || 0);
    })
    //最终实缴金额
    ...
    if (this.state.editMode == "OrderClassDirection") {
      ...
      totalAmount = this.state.fee_list[0].totalProductAmount
    }
    ...
    for (var i = 0; i < this.state.term_list.length - 1; i++) {
      ...
      if (item.term == term) {
        item.payableAmount = parseFloat(value || 0).toFixed(2);
        curr = item;
      }
  ...
  onSave = () => {
    ...
    this.props.form.validateFields((err, values) => {
      if (!err) {
        ...
        values.createDate = formatMoment(values.createDate,'YYYY-MM-DD HH:mm:ss');
  ...
  onSubmit = (values) => {
    ...
    this.props.form.validateFields((err, values) => {
      if (!err) {
        this.props.changeEeqError(); 
        that.setState({ loading: true ,liveOnff:true});
        values.createDate = formatMoment(values.createDate,'YYYY-MM-DD HH:mm:ss');
        //用于 判断是否跳转缴费单页
        this.checkAndFillData(values, function (values) {
          if (values) {
  ...
  disabledBeginDate = (endValue) => {
    let timeRecruit = '';
    if(this.state.recruit_data_list.length){
      timeRecruit = timestampToTime(this.state.recruit_data_list[0].beginDate).split('-');
    }
      let startValue = new Date(timeRecruit[0],timeRecruit[1]-1,timeRecruit[2]);
      if (!endValue || !startValue) {
          return false;
      }
      return endValue.valueOf() >= startValue.valueOf();
  }
  onSetCreateDate = (value) => {
    this.setState({
      createDate: formatMoment(value,'YYYY-MM-DD HH:mm:ss')
    })
  }
  render() {
    ...
    var content =
      <Form>
        ...
        <ContentBox titleName='订单基本信息部分' hideBottomBorder={true}>
          ...
          <div className="dv_split"></div>
          <Row gutter={24} style={{ width: "90%" }} className='createDate-lable'>
            <Col span={12}>
                <FormItem {...searchFormItemLayout16} label="订单创建日期">
                  {getFieldDecorator('createDate', {
                    initialValue: this.state.dataModel.createDate?dataBind(timestampToTime(this.state.dataModel.createDate, true), true):dataBind(timestampToTime(new Date(2018,7,1,0,0,0).getTime(), true), true),
                    rules: [{ required: true, message: '请选择订单创建日期!' }],
                  })(
                    <DatePicker
