...
class CourseArrangeDetail extends React.Component {
  ...
  columns = [
    ...
    },
    {
        ...
        key: 'action',
        render: (text, record) => {
          if(this.state.detail.status != 2 ){
            return <Button onClick={() => { this.onLookView('Edit', record); }}>编辑</Button>
          }else{
            return <Button disabled onClick={() => { this.onLookView('Edit', record); }}>编辑</Button>
          }
        },
    }
  ];
      ...
      case 'Manage':
        ...
        var rowSelection = {
            ...
            getCheckboxProps: record => ({
              disabled: this.state.detail.status != 2 ? false : true,
            })
        }
        block_content = (
          <div>
            ...
            <div className="dv_split">
              {
                this.state.detail.status != 2 ?
                  <Button type="primary" loading={this.state.loading} icon="save" onClick={this.onAdd}>新增课表</Button>
                :
                  <Button type="primary" disabled loading={this.state.loading} icon="save" onClick={this.onAdd}>新增课表</Button>
              }
              
              <span className="split_button"></span>
              {
                (this.state.detail.status != 2 && this.state.UserSelecteds.length) ?
