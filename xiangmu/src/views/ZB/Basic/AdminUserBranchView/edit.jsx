...
import { SettingUserSourcesList,SettingUserSources } from '@/actions/base';
...
class AdminUserBranchView extends React.Component {
    ...
    onSubmit = () => {
        ...
        // if (b.length == 0) { message.warning('必须要勾选一个分部！'); return }
        this.props.form.validateFields((err, values) => {
            if (!err) {  
                ...
                this.props.SettingUserSources({ userId: this.state.dataModel.user.userId, branchIds: b.length >= 1 ? b.join(',') : '' }).payload.promise.then((response) => {
    ...
    fetchUserOrgList = (userId) => {
        ...
        this.props.SettingUserSourcesList(condition).payload.promise.then((response) => {
            let data = response.payload.data;
            let d = [];
            if (data.result === false) {
                message.error(data.message);
                ...
                this.setState({
                    typeChecked1: data.data.userSourceList.filter(a => a.createUid == 1).length > 0,
                    typeChecked2: data.data.userSourceList.filter(a => a.createUid == 2).length > 0,
                    currentUserOrgList1: data.data.userSourceList.filter(a => a.createUid == 1).map((a) => a.orgId.toString()),
                    currentUserOrgList2: data.data.userSourceList.filter(a => a.createUid == 2).map((a) => a.orgId.toString()),
    ...
    onChangeType1 = (e) => {
        ...
        if(e.target.checked){ 
            ...
            regions.map((item) => {
                let funList = this.state.all_org_list; 
                    funList.filter(a => a.parentOrgid == item.orgId && a.orgType == 3 && a.branchType == 1).map((item1) => { 
                        arr.push(item1.orgId.toString())
                    }) 
            })
        }  
        this.setState({ currentUserOrgList1: arr,typeChecked1: e.target.checked });
    }
    onChangeType2 = (e) => {
        let arr = []; 
        if(e.target.checked){ 
        ...
        this.setState({ currentUserOrgList2: arr,typeChecked2: e.target.checked });
    }
    //多种模式视图处理
...
function mapDispatchToProps(dispatch) {
    return {
        ...
        //列表
        SettingUserSourcesList: bindActionCreators(SettingUserSourcesList, dispatch),
        //编辑
        SettingUserSources: bindActionCreators(SettingUserSources, dispatch),
