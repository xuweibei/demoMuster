...
class ProductEditView extends React.Component {
    ...
    componentWillMount() { 
        this.loadBizDictionary([ 'teachMode','class_type_type','category_scope' ]);
        if(this.props.editMode == 'Edit2'){
            this.setState({
                typeName:this.state.dataModel.classTypeTypeName
            })
        }
        //修改模式下，级联触发恢复选项
    ...
    fetchClassTypeList = (itemIds) => {
        ...
        this.props.getClassList(condition).payload.promise.then((response) => {
            ...
            if (data.result === false) {
                ...
                if(this.state.dataModel.classTypeId){
                    this.setState({
                        typeName:data.data.find(item=>item.classTypeId == this.state.dataModel.classTypeId ).classTypeTypeName
                    })
                }
                this.setState({ dic_ClassTypes: data.data.map((a) => { return { title: a.classTypeName, value: a.classTypeId.toString(),classTypeTypeName:a.classTypeTypeName } }) })
            }
        })
    }
    //检索科目列表数据
    ...
    ClassSwitching = (value) => {  
        this.setState({
            typeName:this.state.dic_ClassTypes.find(item=>item.value == value ).classTypeTypeName
        })
                            ...
                            <Row gutter={24}>
                                ...
                                <Col span={12}>
                                    ...
                                    >
                                        {getFieldDecorator('classTypeId', {
                                            ...
                                            <Select
                                                onChange={(data)=>this.ClassSwitching(data)}
                                            >
                                ...
                                <Col span={12}>
                                    <FormItem
                                            ...
                                            label="科目范围"
                                        >
                                            {getFieldDecorator('categoryScope', {
                                                initialValue: this.state.dataModel.product?dataBind(this.state.dataModel.product.categoryScope):'',
                                                rules: [{
                                                    required: true, message: '请选择科目范围!',
                                                }],
                                            })(
                                                <Select>
                                                    <Option value=''>--请选择--</Option>
                                                    {this.state.category_scope.map((item) => { 
                                ...
                                <Col span={12} >
                                    ...
                                    >
                                        {getFieldDecorator('remoteType', { 
                                            initialValue: this.state.dataModel.remoteType || '',
                                            rules: [{
                                                required: true, message: '请选择异地上课!',
                                            }],
                                            })(
                                            <Select>
                                                <Option value="">全部</Option>
                                                {this.props.remote_type.map((item, index) => {   
                                                    return <Option value={Number(item.value)} key={index}>{item.title}</Option>
                                                })}
                                            </Select>
                                        )}
                                    </FormItem>
                                ...
                                {(this.state.productType == 2 && this.state.teachMode == 2) && <Col span={12}>
                                    ...
                                    >
                                        {getFieldDecorator('onlineCourseActiveType', {
                                            ...
                                            <RadioGroup>
                                                <Radio value='1'>按课程商品</Radio>
                                                <Radio value='2'>按课程</Radio>
                                            </RadioGroup>
                                            )}
                                    </FormItem>
                                </Col>
                                ...
                                {this.state.productType == '2' && <Col span={12}>
                                    <FormItem
                                        ...
                                        label="参考价格"
                                    >
                                        {getFieldDecorator('courseStandardPrice', {
                                            initialValue: dataBind(this.state.dataModel.courseStandardPrice)
                                        })(
                                            <InputNumber min={0} step={1} />
                                            )}
                                    </FormItem>
                                ...
                                {this.state.productType == '2' && <Col span={12}>
                                    <FormItem
                                        ...
                                        label="参考课时"
                                    >
                                        {getFieldDecorator('classStandardHour', {
                                            initialValue: dataBind(this.state.dataModel.classStandardHour)
                                        })(
                                            <InputNumber min={0} step={1} />
                                            )}
                                    </FormItem>
                                ...
                                {
                                   this.props.editMode == 'Edit' && this.state.dataModel.product && this.state.dataModel.product.productType == 1 && <Col span={12}>
                                    <FormItem
                                        {...searchFormItemLayout}
                                        label="授课方式"
                                    >
                                        <span>{getDictionaryTitle(this.state.teachMode,this.state.dataModel.product.teachMode)}</span>
                                    </FormItem>
                                </Col>
                                }
                    ...
                    block_content = (
                        <Form layout="Vertical">
                            <Row gutter={24}>
                                ...
                                { this.state.dataModel.product.productType == 1 && <Col span={12}>
                                    <FormItem
                                        {...searchFormItemLayout}
                                        label="授课方式"
                                    >
...
const mapStateToProps = (state) => {
    ...
    return { dic_MyItems, dic_ClassTypes,Dictionarys };
};
