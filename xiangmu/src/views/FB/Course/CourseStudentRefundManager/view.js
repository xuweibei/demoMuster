...
import { getCourseStudentRefundManagerById,CheckDetails } from '@/actions/course';
...
class CourseStudentRefundManagerView extends React.Component {
    ...
    componentWillMount() {
        ...
        if (this.props.currentDataModel.studentChangeId) {
            this.props.getCourseStudentRefundManagerById({ studentChangeId: this.props.currentDataModel.studentChangeId }).payload.promise.then((response) => {
    ...
    render() {
        ...
        block_content = (
            <Form>
                <Row gutter={24}>
                    <Col span={24}>
                        <span style={{ color: 'red' }}>{this.state.dataModel.changeTypeName}申请详细信息</span>
                        </Col>
                    <Col span={12}>
                        <FormItem {...searchFormItemLayout} label="退费订单号">
                    ...
                    <Col span={12}>
                        <FormItem {...searchFormItemLayout} label="手机号">
                            {this.state.dataModel.oldStudentMobile}
                        </FormItem>
                    </Col>
                    <Col span={12}>
                    ...
                    <Col span={12}>
                        <FormItem {...searchFormItemLayout} label="申请课程退费金额(¥)">
                            <span style={{ color: 'red' }}>{formatMoney(this.state.dataModel.refundRealCourseAmountTotal)}</span>
                            &nbsp;&nbsp;&nbsp;
                            <a onClick={() => {
                    ...
                    <Col span={12}>
                        <FormItem {...searchFormItemLayout} label="申请临时缴费金额退费(¥)">
                            <span style={{ color: 'red' }}>{formatMoney(this.state.dataModel.refundRealBalanceTotal)}</span>
                        </FormItem>
                    </Col>
                    <Col span={12}>
                        <FormItem {...searchFormItemLayout} label="申请退费总金额(¥)">
                            <span style={{ color: 'red' }}>{formatMoney(this.state.dataModel.refundRealAmountTotal)}</span>
                        </FormItem>
                    </Col>
                    {
                    ...
                    <Col span={24}>
                        <FormItem {...searchFormItemLayout24} label="退费日期">
                            {this.state.dataModel.oldOperateDate}
                        </FormItem>
                    </Col>
                    <Col span={12}>
                    ...
                    <Col span={12}>
                        <FormItem {...searchFormItemLayout} label="申请人/申请日期">
                            {this.state.dataModel.proposer}
                            &nbsp;&nbsp;
                            {this.state.dataModel.proposerDate}
                        </FormItem>
                    </Col>
                    <Col span={24}>
                        以下为申请审核情况
                    ...
                    {
                        this.props.currentDataModel.status == 2 && <div>
                            <Col span={24}>
                                以下为申请审核进度
                            </Col>
                            {
        ...
        return (
            <div>
                {!this.state.showList && !this.state.details && !this.state.orderNum && <ContentBox titleName={title} bottomButton={this.renderBtnControl()}>
                ...
                {
                    ( this.state.orderNum && !this.state.details   ) &&  <OrderDetailView viewCallback={this.onViewCallback}
                    studentId={this.props.currentDataModel.oldStudentId}
                    orderId={this.props.currentDataModel.oldOrderId}
                    tab={3}
                />
                }
                {this.state.showList && !this.state.details && !this.state.orderNum &&
                    <Row>
                        <Col span={24}>
...
function mapDispatchToProps(dispatch) {
    return {
        //基本字典接口
        getCourseStudentRefundManagerById: bindActionCreators(getCourseStudentRefundManagerById, dispatch),
