...
class StudentPayfeeCustomersImprot extends React.Component {
  ...
  onExport = () => {
    ...
    var url = `${env.serverURL}/edu/intentionUniversities/importIntentionUniversities?token=${env.getToken()}`;
  ...
  onSave = () => {
    ...
    const successFn = (response) => { 
      ...
      if (data.state == 'success') {
        ...
        that.setState({
          ...
          error_url: `/edu/intentionUniversities/exportErrIntentionUniversities`,
        })
        //param.success({
    ...
    let uploadUrl = `${env.serverURL}/edu/intentionUniversities/importIntentionUniversities?token=${token}`;
    const xhr = new XMLHttpRequest
  ...
  render() {
              ...
              <Row type='flex' justify='center'>
                <div style={{ marginBottom: 20 }}>
                  <FileDownloader
                    ...
                    method={'post'}//提交方式
                    options={{ filePath: 'upload/importTemplate/intentionUniversitiesImport.xlsx' }}
                    title={'下载模板'}
                  >
                  </FileDownloader>
                </div>
    ...
    return (
      <ContentBox titleName={"大客户意向高校情况导入"} bottomButton={
        <FormItem
