...
class PsignImport extends React.Component {
  constructor(props){
    super(props);
    this.state= {
      editMode: '',
  ...
  onSave = () => {
    ...
    let uploadUrl = `${env.serverURL}/edu/order/uploadFile?token=${token}`;
  ...
  render(){
    ...
    return (
      <ContentBox titleName={"纸质合同上传"} bottomButton={
        <FormItem
        ...
        {
          this.props.currentDataModel.esignStatus && this.props.currentDataModel.psignFile ?
          <div>
            <div className="dv_split"></div>
            点击查看已上传的合同：<a href={this.props.currentDataModel.psignFile} target='_Blank'>查看纸质合同</a>
          </div>
          : ''
        }
...
//表单组件 封装
const WrappedManage = Form.create()(PsignImport);
