...
import { getCourseStudentMoveManagerById } from '@/actions/course';
...
class CourseStudentMoveManagerView extends React.Component {
    constructor(props) {
        super(props)
        this.state = {
            dataModel:
    ...
    componentWillMount() {
        if (this.props.currentDataModel.studentChangeId) {
            this.props.getCourseStudentMoveManagerById({ studentChangeId: this.props.currentDataModel.studentChangeId }).payload.promise.then((response) => {
        ...
        block_content = (
            <Form>
                <Row gutter={24}>
                    ...
                    <Col span={24}>
                        <span style={{ color: 'red' }}> 以下为申请审核情况</span>
                        </Col>
                    <Col span={24}>
...
//表单组件 封装
const WrappedManage = Form.create()(CourseStudentMoveManagerView);
...
function mapDispatchToProps(dispatch) {
    return {
        //基本字典接口
        getCourseStudentMoveManagerById: bindActionCreators(getCourseStudentMoveManagerById, dispatch),
    };
}
//redux 组件 封装
