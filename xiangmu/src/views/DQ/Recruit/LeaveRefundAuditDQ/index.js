    ...
    },
    {
        ...
        render: (text, record) => {
            return <DropDownButton>
                {record.statusNum != 5 && <Button onClick={() => {
                    this.onLookView('View', record)
                }}>查看</Button>
                }
                {record.statusNum == 5 && <Button onClick={() => {
                    this.props.getCourseStudentRefundCheckStudentChange({ studentChangeId: record.studentChangeId,auditOrgType:3 }).payload.promise.then((response) => {
                                            ...
                                            <FormItem {...searchFormItemLayout} label="审核状态">
                                                {getFieldDecorator('status', {
                                                    ...
                                                    <Select >
                                                        ...
                                                        {this.state.student_change_refund_status_front.filter(item=> item.value!=1 && item.value!=2 && item.value!=3 && item.value!=4 ).map((item, index) => { 
