...
class RoleView extends React.Component {
    ...
    renderTree(menus, isPower) {
        ...
        return menus.map((item) => {
            //如果是查看角色权限，则以角色功能列表为准（避免当前用户权限少导致显示不完整），
            //如果是设置角色权限，则以当前用户对应的最大权限为准
            let funList = isPower ? this.state.allFuns : this.state.allRoleFuns;
            let subMenu = funList.filter(a => a.parentId == item.funId).sort((a, b) => { return a > b ? 1 : 0; });
            let childs = this.renderTree(subMenu, isPower);
    ...
    renderEditModeOfView() {
        ...
        switch (this.props.editMode) {
            ...
            case "Power":
                {
                    //当前用户对应的功能菜单
                    let firstMenus = this.state.allFuns.filter(a => a.parentId == null).sort((a, b) => { return a > b ? 1 : 0 });
            ...
            case "Delete":
                {
                    //当前用户对应的功能菜单
                    let firstMenus = this.state.allRoleFuns.filter(a => a.depth == 1).sort((a, b) => { return a > b ? 1 : 0 });
                    let block_funTree = this.renderTree(firstMenus, 0);
                    block_content = (
