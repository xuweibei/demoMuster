...
class StudentPayfeeCustomersImprot extends React.Component {
  ...
  onCancel = () => {
    if(this.props.onOffNum){
      this.props.viewCallbackNum();
    }else{
  ...
  onExport = () => {
    ...
    var url = `${env.serverURL}/edu/callcenterTask/importExcel?token=${env.getToken()}`;
  ...
  onSave = () => {
    ...
    formData.append('regionId',this.props.currentDataModel.regionId)
    formData.append('callcenterTaskId',this.props.currentDataModel.callcenterTaskId);
    this.setState({
      uploading: true,
      isImported: false
    });
    ...
    const successFn = (response) => {
      ...
      if (data.state == 'success') {
        ...
        that.setState({
          ...
          error_url: `/edu/callcenterTask/exportErr`,
        })
        //param.success({
    ...
    let uploadUrl = `${env.serverURL}/edu/callcenterTask/importExcel?token=${token}`;
    const xhr = new XMLHttpRequest
  ...
  render() {
              ...
              <Row gutter={24} type='flex' justify='center'>
                ...
                <div style={{ marginBottom: 20 }}>
                  <FileDownloader
                    ...
                    method={'post'}//提交方式
                    options={{ filePath: 'upload/importTemplate/taskStuUpload.xlsx' }}
                    title={'下载模板'}
                  >
                  </FileDownloader>
                </div>
    ...
    return (
      <ContentBox titleName={"上传外呼任务学生"} bottomButton={
        <FormItem
