...
class OrganizationSetUserView extends React.Component {
    constructor(props) {
        super(props)
        //扩展方法用于本组件实例
        ...
        this.state = {
            ...
            pagingSearch: {
                ...
                realName: '',
                department: '',
                areaId: props.currentDataModel.orgId,
    ...
    onSubmit = () => {
        this.props.viewCallback({ areaId: this.state.pagingSearch.areaId, orgName: this.state.dataModel.orgName });
    }
    onCancel = () => {
    ...
    onBatchDelete = () => {
        Modal.confirm({
            title: '确定将所选用户从本区域中删除?',
            content: '点击确认删除所选用户!否则点击取消！',
            onOk: () => {
                this.props.deleteUserByUsersId({ userIds: this.state.UserSelecteds.join(','), areaId: this.state.pagingSearch.areaId }).payload.promise.then((response) => {
    ...
    onDelete = (userId) => {
        Modal.confirm({
            ...
            content: '点击确认删除所选用户!否则点击取消！',
            onOk: () => {
                this.props.deleteUserByUsersId({ userIds: userId, areaId: this.state.pagingSearch.areaId }).payload.promise.then((response) => {
                    let data = response.payload.data;
                    if (data.result === false) {
                        message.error(data.message);
                    }
...
//表单组件 封装
const WrappedOrganizationSetUserView = Form.create()(OrganizationSetUserView);
...
//redux 组件 封装
export default connect(mapStateToProps, mapDispatchToProps)(WrappedOrganizationSetUserView);
