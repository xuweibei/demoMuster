...
import { canOrderStudentQuery, studentCreate, studentUpdate,
  createOrder,studentUpdateOnline,studentCreateOnline
    ...
    if(!dataModel){
      ...
      switch (this.state.editMode) {
        ...
        //case 'EditByStudentManage'
          this.props.studentUpdate(dataModel).payload.promise.then((response) => {
              ...
              if (data.state === 'error') {
                  ...
                  //this.fetch();
                  //进入管理页
                  console.log(this.state.editMode.replace("Edit", "Order"))
        ...
        case 'OrderPersonOnline':
          this.props.createOrder(dataModel).payload.promise.then((response) => {
              let data = response.payload.data;  
              if (data.state === 'error') { 
                  message.error(data.message);
                  this.setState({
                  ...
                  if(dataModel.orderStatus == 0){
                    ...
                    //   this.onLookView("Manage");
                    // }
                    if((dataModel.payeeType == 1 || dataModel.payeeType == 2) && dataModel.paymentStatus){
                      this.onNextView("Fee", dataModel);
                    }else {
                      this.onLookView("Manage");
                    }
          ...
          <div>
            <SearchForm
              ...
              form_item_list={[
                ...
                {type:'input', name: 'benefitFconsultUserName', title: '面咨人员', value: this.state.pagingSearch.benefitFconsultUserName},
                {type:'input', name: 'benefitPconsultUserName', title: '电咨人员', value: this.state.pagingSearch.benefitPconsultUserName},
                {type:''}
              ]}
