...
//业务接口方法引入
import { adminHomeModule, adminHomeModuleAdd, adminHomeModuleEdit, adminHomeModuleDel } from '@/actions/admin';
...
class HomeManage extends React.Component {
    ...
    fetch = (params = {}) => {
        ...
        this.props.adminHomeModule(this.state.pagingSearch).payload.promise.then((response) => {
    ...
    onViewCallback = (dataModel) => {
        if (!dataModel) {
            ...
            switch (this.state.editMode) {
                case "Create":
                    this.props.adminHomeModuleAdd(dataModel).payload.promise.then((response) => {
                        let data = response.payload.data;
                    ...
                    break;
                case "Edit": //提交
                    this.props.adminHomeModuleEdit(dataModel).payload.promise.then((response) => {
                        let data = response.payload.data;
                        if (data.state != 'success') {
                            message.error(data.message, 3);
                        }
        ...
        },
        {
            ...
            render: (text, record) => (
                <DropDownButton>
                    ...
                    <Button onClick={() => { this.onDeltet(record.homeAreaModuleId) }}>删除</Button>
                </DropDownButton>
            ),
        }]
    ...
    onDeltet = (moduleId) => {
        Modal.confirm({
            ...
            onOk: () => {
                ...
                this.props.adminHomeModuleDel(params).payload.promise.then((response) => {
                    let data = response.payload.data;
                    if (data.state != 'success') {
                        this.setState({ loading: false })
...
function mapDispatchToProps(dispatch) {
    return {
        ...
        adminHomeModuleAdd: bindActionCreators(adminHomeModuleAdd, dispatch),
        adminHomeModuleEdit: bindActionCreators(adminHomeModuleEdit, dispatch),
        adminHomeModuleDel: bindActionCreators(adminHomeModuleDel, dispatch),
    };
}
//redux 组件 封装
