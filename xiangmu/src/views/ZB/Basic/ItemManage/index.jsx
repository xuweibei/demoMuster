...
class ItemManage extends React.Component {
    ...
    componentWillMount() {
        //载入需要的字典项
        this.loadBizDictionary(['dic_Status', 'channel', 'dic_IsSee']);
        //首次进入搜索，加载服务端字典项内容
        this.onSearch();
    }
    ...
    },
    {
        ...
        render: (text, record, index) => {
            return getDictionaryTitle(this.state.channel, record.channelId);
        }
    },
    {
        title: '状态',
    ...
    render() {
        ...
        switch (this.state.editMode) {
            ...
            default:
                //除查询外，其他扩展按钮数组
                let extendButtons = [];
                extendButtons.push(<Button onClick={() => { this.onLookView('Create', { ApplicationID: this.state.defaultApplicationID, Status: 1, itemName: '', Description: '' }) }} icon="plus" className="button_dark">项目</Button>);
                block_content = (<div>
                    {/* 搜索表单 */}
...
//表单组件 封装
const WrappedItemManage = Form.create()(ItemManage);
...
//redux 组件 封装
export default connect(mapStateToProps, mapDispatchToProps)(WrappedItemManage);
