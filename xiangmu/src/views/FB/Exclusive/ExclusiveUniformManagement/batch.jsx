...
import { getUserByTeachList } from '@/actions/base';
import { StudentsExclusiveBatch } from '@/actions/stuService';
...
class View extends React.Component {
    ...
    fetch = () => {
        this.props.getUserByTeachList({teachCenterId:this.props.selectDataList[0].teachCenterId,currentPage: 1,pageSize: 30}).payload.promise.then((response)=>{
    ...
    handleSearch = (value) => { 
        ...
        if (value && value.indexOf(' ')!=0) {
            ...
            this.props.getUserByTeachList({realName:value,teachCenterId:this.props.selectDataList[0].teachCenterId}).payload.promise.then((response) => {
    ...
    onSubmit = () => {
        ...
        this.props.form.validateFields((err, values) => {
            if (!err) {
                ...
                values.ids = ids.join(',')
                values.privateTeacherId = this.state.tags[0].userId;  
                this.props.StudentsExclusiveBatch(values).payload.promise.then((response) => {
                    let data = response.payload.data;
                    if (data.state === 'success') { 
    ...
    render(){
        ...
        switch(this.props.editMode){
            ...
            extendButtons.push(
                ...
                block_content = (
                    <div>
                        <ContentBox titleName='专属学服批量设置' bottomButton={this.renderSearchBottomButtons(extendButtons,'l','r')}>
                            <Form
                                className="search-form"
                            >
                                <Row gutter={24}  type="flex">  
                                    <Col span={24}>
                                        <div style={{marginBottom:'10px',fontWeight:'800'}}>您批量选择了<span style={{color:'red',margin:'0 6px'}}>{this.props.selectDataList.length?this.props.selectDataList[0].teacherCenterName:''}</span>教学点<span style={{color:'red',margin:'0 6px'}}>{this.props.selectDataList.length?this.props.selectDataList.length:''}</span>个学生，进行专属学服的设置 ，请选择学服老师【支持按姓名模糊查询】</div>
                                    </Col>
                                    <Col span={6}></Col>
                                    <Col span={12}>
                                        <FormItem {...searchFormItemLayout} label={'学服老师'}>
                                            {getFieldDecorator('classTeacherId',{
                                                initialValue:''
                                            })(
                                                <div>
                                                    {this.state.tags.map((tag, index) => {
                                                        const isLongTag = tag.realName.length > 20;
                                                        const tagElem = (
                                                            <Tag value={tag.userId} key={tag.userId} closable afterClose={() => this.handleClose(tag)}>
                                                                {isLongTag ? `${tag.realName.slice(0, 20)}...` : tag.realName}
                                                            </Tag>
                                                        );
...
function mapDispatchToProps(dispatch){
    return {
        ...
        StudentsExclusiveBatch:bindActionCreators(StudentsExclusiveBatch,dispatch)
    }
}
