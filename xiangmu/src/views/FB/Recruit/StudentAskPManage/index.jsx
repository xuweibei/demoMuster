...
import { message, Form, Row, Col, Input, Select, Button, Modal, Table, Pagination, Divider, InputNumber, DatePicker } from 'antd';
...
import { getStudentAskPList,studentUpdateInActive } from '@/actions/recruit';
//业务数据视图（增、删、改、查)
import ByStudentId from '../StudentAskPManage/byStudentId.jsx';
...
class StudentAskPManage extends React.Component {
    ...
    constructor(props) {
        ...
        let day = startTimeYear+'-'+startTimeMonth+'-'+startTimeDay
        this.state = {
            visible:false,
            showHide:props.state.zs.ll == 'menu81'?true:false,
            ...
            pagingSearch: {
                ...
                regRegionId: '',
                askType: 1,//电咨类型
                nextAskDateStart:props.state.zs.ll == 'menu81'?startTimeYear+'-'+startTimeMonth+'-'+startTimeDay:'',
                nextAskDateEnd:props.value == 'oneDay'?day:props.value == 'threeDay'?this.getDay(2):'',
                userCreateDateStart:'',
                userCreateDateEnd:'',
                askDateStart:'',
                askDateEnd:'',
    ...
    componentWillMount() { 
        //载入需要的字典项
        this.loadBizDictionary(['dic_Status', 'channel', 'sex', 'reg_source', 'grade', 'order_status','dic_YesNo','ask_source']);
        //首次进入搜索，加载服务端字典项内容
        // this.onSearch();  
    }
    ...
    //table 输出列定义
    // 按学生查询：区域，学生姓名，性别，在读高校，手机号，微信号，QQ，学生来源，学生意向等级，学生订单情况，分配日期，最新电咨日期，下次电咨日期，距今天数，详细信息
    columns = [
        {
            title: '学生姓名',
            fixed: 'left',
        ...
        },
        {
            ...
            dataIndex: 'gender',
            render: (text, record, index) => {
                var gender = record.gender == 0 ? 1 : record.gender
                return getDictionaryTitle(this.state.sex, gender);
            }
        },
        {
            title: '在读高校',
        ...
        },
        {
            ...
            dataIndex: 'regSource',
            render: (text, record, index) => {
                var regSource = record.regSource == 0 ? 1 : record.regSource
                return getDictionaryTitle(this.state.reg_source, regSource);
            }
        },
        {
            title:'最近参加活动',
        ...
        },
        {
            ...
            dataIndex: 'grade',
            render: (text, record, index) => {
                var grade = record.grade == 0 ? 1 : record.grade
                return getDictionaryTitle(this.state.grade, grade);
            }
        },
        {
            title: '学生订单情况',
        ...
        },
        {
            ...
            render: (text, record, index) => {
                return timestampToTime(record.benefitPconsultDate)
            },
        },
        {
            title: '最新电咨日期',
        ...
        },
        {
            ...
            render: (text, record) => {
                return <DropDownButton>
                    <Button onClick={() => { this.onLookView('ByStudentId', record) }}>编辑电咨</Button>
                    <Button onClick={() => { this.onLookView('EditByStudentManage', record) }}>编辑学生</Button>
                </DropDownButton>
            },
        }];
    ...
    fetch = (params = {}) => { 
        ...
        condition.currentPage = condition.currentPage;//修正分页参数 getStudentAskPList getItemList
        let askDateStart;
        let askDateEnd;
        ...
        let fpDateStart = condition.fpDateStart;
        ...
        if(fpDateStart){
            condition.fpDateStart = formatMoment(fpDateStart[0]);
            condition.fpDateEnd = formatMoment(fpDateStart[1]);
        }
        ...
        this.props.getStudentAskPList(condition).payload.promise.then((response) => {
    ...
    render() {
        ...
        switch (this.state.editMode) {
            ...
            case "View":
            case "Delete":
                block_content = <ItemView viewCallback={this.onViewCallback} {...this.state} />
                break; 
            case "ByStudentId":
                        ...
                        {!this.state.seachOptionsCollapsed &&
                            <Form className="search-form" >
                                <Row justify="center" gutter={24} align="middle" type="flex">
                                    ...
                                    <Col span={12} >
                                        <FormItem {...searchFormItemLayout} label={'？天内未电询'} >
                                            {getFieldDecorator('numberOfDaysAgo', { initialValue: this.state.pagingSearch.numberOfDaysAgo })(
                                                <InputNumber min='1' />
                                            )}
                                        </FormItem>
                                    </Col>
                                    ...
                                    <Col span={12} >
                                        <FormItem {...searchFormItemLayout}
                                            label="是否有咨询记录">
                                            {getFieldDecorator('isHasAsk', { initialValue: this.state.pagingSearch.isHasAsk })(
                                                <Select>
                                                    <Option value="">全部</Option>
                                                    {this.state.dic_YesNo.map((item, index) => {
                                                        return <Option value={item.value} key={index}>{item.title}</Option>
                                    ...
                                    <Col span={12}>
                                        <FormItem
                                            {...searchFormItemLayout}
                                            label="分配时间">
                                            {getFieldDecorator('fpDateStart', { initialValue: this.state.pagingSearch.fpDateStart?[moment(this.state.pagingSearch.fpDateStart,dateFormat),moment(this.state.pagingSearch.fpDateEnd,dateFormat)]:[] })(
                                                <RangePicker style={{width:220}}/>
                                            )}
                                        </FormItem>
                                    </Col>
...
const mapStateToProps = (state) => {
    ...
    let { value } = state.stateValue;
    return { Dictionarys, currentUser,state,value };
};
...
function mapDispatchToProps(dispatch) {
    return {
        ...
        getStudentAskPList: bindActionCreators(getStudentAskPList, dispatch),
