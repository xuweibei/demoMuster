...
viewCallback [回调]
*/
class AdminUserItemView extends React.Component {
    constructor(props) {
        super(props)
        this.state = {
    ...
    onSubmit = () => {
        ...
        this.props.form.validateFields((err, values) => {
            if (!err) {
                ...
                this.props.viewCallback({userIds: this.state.dataModel.userIds.join(','),...values });//合并保存数据
            }
        });
    }
    ...
    renderBtnControl() {
        if (this.props.editMode != 'View') {
            return <FormItem
                ...
                {...btnformItemLayout}
            >
                <Button type="primary" loading={this.state.loading} icon="save" onClick={this.onSubmit}>{getViewEditModeTitle(this.props.editMode=='EditItem'?'Edit':this.props.editMode)}</Button><span className="split_button"></span><Button icon="rollback" onClick={this.onCancel} >取消</Button>
            </FormItem>
        }
        else {
            return <FormItem
...
const WrappedAdminUserItemView = Form.create()(AdminUserItemView);
...
//redux 组件 封装
export default connect(mapStateToProps, mapDispatchToProps)(WrappedAdminUserItemView);
