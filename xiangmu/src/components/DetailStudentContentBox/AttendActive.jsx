...
import { queryByStudentId} from '@/actions/recruit';
//业务数据视图（增、删、改、查)
...
class AttendActive extends React.Component {
    columns = [
        ...
        },
        {
            title: '活动开始时间',
            dataIndex: 'startTime',
            render: (text, record, index) => (`${timestampToTime(record.startTime)}`)
        },
        {
            title: '活动结束时间',
            dataIndex: 'endTime',
            render: (text, record, index) => (`${timestampToTime(record.endTime)}`)
        },
    ]
    constructor(props) {
        super()
    ...
    fetch = () => {
        ...
        this.props.queryByStudentId(condition).payload.promise.then((response) => {
    ...
    render() {
        ...
        let block_content_1 =
            <ContentBox titleName='参加活动' hideBottomBorder={false}>
...
function mapDispatchToProps(dispatch) {
    return {
        ...
        queryByStudentId:bindActionCreators(queryByStudentId, dispatch),
    };
}
//redux 组件 封装
