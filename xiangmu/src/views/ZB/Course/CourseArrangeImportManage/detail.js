  ...
  fetch(){
      ...
      this.props.courseArrangeByIdQuery(this.state.detail.courseArrangeId).payload.promise.then((response) => {
          ...
          if (data.state === 'success') {
            ...
            if(data.data.headParam){ 
              this.setState({
                pcName:data.data.headParam.courseplanBatchName
              })
            }
            this.setState({
              data_list: list,
        ...
        block_content = (
          <div>
            <SearchForm
              num_column={2}
              form_item_list={[
                {type:'text', value: this.state.detail.itemName, title: '项目'},
                {type:'text', value: this.state.pcName, title: '开课批次'},
