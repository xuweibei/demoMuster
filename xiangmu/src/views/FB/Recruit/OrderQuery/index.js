...
import { queryOrder, getContractUrl } from '@/actions/recruit';
...
class OrderQuery extends React.Component {
  ...
  columns = [
    ...
    {
        title: '首次缴费时间',
        dataIndex: 'firstPayeeDate',
        render:(text,record)=>{ 
          return timestampToTime(record.firstPayeeDate)
        }
    },
    {
        title: '操作',
        ...
        render: (text, record) => (
          ...
          <DropDownButton>
              <Button onClick={() => { this.onLookView('ViewOrder', record); }}>查看</Button>
              {
                ...
                <Button onClick={() => { this.onGetQrCode(record,2); }}>电子签重签</Button>
                : ''
              }
              {
                this.props.orgType == 1 && !record.psignFile && (( record.esignStatus == 0 )?<Button onClick={() => { this.onGetQrCode(record,1); }}>电子签查看</Button> : <Button><a style={{fontSize:12+'px',marginLeft:2+'px'}} href={record.esignFile} target='_Blank'>电子签查看</a></Button>)
              }
              {
                this.props.orgType == 1 && !record.esignFile && record.psignFile && <Button><a style={{fontSize:12+'px',marginLeft:2+'px'}} href={record.psignFile} target='_Blank'>纸质合同查看</a></Button>
              }
  ...
  fetch(params){ 
      ...
      this.props.queryOrder(condition).payload.promise.then((response) => {
          let data = response.payload.data;
          if (data.state === 'success') {
            var list = data.data;
            //list.push(data.data);
  ...
  render(){  
    ...
    switch (this.state.editMode) {
      ...
      extendButtons.push(<FileDownloader
        apiUrl={'/edu/order/exportListByQuery'}//api下载地址
        method={'POST'}//提交方式
                    ...
                    <Col span={12}>
                      <FormItem {...searchFormItemLayout} label={'订单状态'} >
                        {getFieldDecorator('orderStatus', { initialValue: this.state.pagingSearch.orderStatus })(
                          <Select>
                            <Option value="">全部</Option>
                            {this.state.order_status.filter(a => a.value == 2 || a.value == 4 || a.value == 5 || a.value== 6 || a.value == 7).map((item, index) => {
                    ...
                    <Col span={12}>
                          <FormItem {...searchFormItemLayout} label={'大客户'}>
                            {getFieldDecorator('partnerId',{ initialValue:this.state.pagingSearch.partnerId})(
                               <Select>
                                 <Option value=''>全部</Option>
                                  {this.state.queryArr.map(item=>{
                                    return <Option value={item.partnerId} title={item.partnerName}>{item.partnerName}</Option>
                                  })}
                                </Select>
                            )}
                          </FormItem>
