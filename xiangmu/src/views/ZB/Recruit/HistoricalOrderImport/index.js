...
class StudentPayfeeCustomersImprot extends React.Component {
  ...
  onExport = () => {
    ...
    var url = `${env.serverURL}/edu/order/importByHistoryPayment?token=${env.getToken()}`;
  ...
  onSave = () => {
    ...
    const successFn = (response) => {
      ...
      if (data.state == 'success') {
        ...
        that.setState({
          ...
          error_url: `/edu/order/exportErrorByHistoryPayment`,
        })
        //param.success({
    ...
    let uploadUrl = `${env.serverURL}/edu/order/importByHistoryPayment?token=${token}`;
    const xhr = new XMLHttpRequest
  ...
  render() {
              ...
              <Row type='flex' justify='center'>
                <div style={{ marginBottom: 20 }}>
                  <FileDownloader
                    ...
                    method={'post'}//提交方式
                    options={{ filePath: 'upload/importTemplate/oldOrderFeeUploadImport.xlsx' }}
                    title={'下载模板'}
                  >
                  </FileDownloader>
                </div>
    ...
    return (
      <ContentBox titleName={"历史订单缴费导入"} bottomButton={
        <FormItem
