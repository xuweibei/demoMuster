...
import {
  partnerProductPriceApplyList, partnerProductPriceApplyAdd, partnerProductPriceApplyUpdate, partnerProductPriceApplyBatchCopy, partnerProductPriceApplyDelete, partnerProductPriceApplyAudit, partnerProductPriceApplyBatchSubmit
...
class ProductPriceApplyManage extends React.Component {
  ...
  onBatchPublish = () => {
    Modal.confirm({
      ...
      content: '请确认',
      onOk: () => {
        this.props.partnerProductPriceApplyBatchSubmit({
          productPriceApplyIds: this.state.UserSelecteds.join(',')
  ...
  render() {
    ...
    switch (this.state.editMode) {
      ...
      default:
        ...
        let extendButtons = [];
        extendButtons.push(<Button onClick={() => { this.onLookView('Copy', {}) }} icon="plus" className="button_dark">复制定价</Button>);
        extendButtons.push(<Button onClick={() => { this.onLookView('Create', { ...this.state.pagingSearch }) }} icon="plus" className="button_dark">新增定价</Button>);
                      ...
                      <FormItem {...searchFormItemLayout} label="状态" >
                        {getFieldDecorator('auditStatus', { initialValue: this.state.pagingSearch.auditStatus })(
                          <Select>
                            ...
                            {this.state.product_partner_price_status.map((item, index) => {
                  ...
                  <Col span={10}>
                    <div className='flex-row'>
                      {(this.state.data.length > 0 && this.state.UserSelecteds.length > 0) ?
                        <Button onClick={this.onBatchPublish} icon='rocket'>提交审核</Button> :
                        <Button disabled icon='rocket'>提交审核</Button>
                      }
                    </div>
                  </Col>
