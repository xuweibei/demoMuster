...
import { selectPayfeeStatement, reportFeePayMonthByDate } from '@/actions/enrolStudent';
...
class PayfeeStatementManage extends React.Component {
  ...
  columns = [
    ...
    },
    {
      title: '月份',
      dataIndex: 'monthDate',
      render: (text, record, index) => (`${record.nian}年${record.yue}月`),
    },
    {
      title: '订单数',
      dataIndex: 'orderNum',
    },
    {
      ...
      render: (text, record, index) => {
        return formatMoney(record.orderAmount);
      }
    },
    {
      title: '当月缴费金额（¥）',
      ...
      render: (text, record, index) => {
        return formatMoney(record.monthPayAmount);
      }
    },
    {
      title: '当月退费金额',
      dataIndex: 'monthRefundAmount',
      render: (text, record, index) => {
        return formatMoney(record.monthRefundAmount);
      }
    },
    {
      title: '操作',
      ...
      render: (text, record) => {
        ...
        return <FileDownloader
          apiUrl={'/edu/studentPayfee/exportPayfeeStatement '}//api下载地址
          method={'post'}//提交方式
          options={{ month: record.yue, year: record.nian,feePayMonthId:record.feePayMonthId}}//提交参数
          title={"导出报表"}
        >
        </FileDownloader>
  ...
  fetch = (params = {}) => {
    ...
    this.props.selectPayfeeStatement(condition).payload.promise.then((response) => {
  ...
  onChangeDate=()=>{
      this.props.form.validateFields((err, values) => {
          if(!err){
            ...
            this.props.reportFeePayMonthByDate({ date:formatMoment(date) }).payload.promise.then((response) => {
                let data = response.payload.data;
                if (data.state == 'error') {
                    message.error(data.message);
                }
                else {
                    message.success('报表已开始在后台生成，请稍后再刷新查看');
                    this.setState({
                      isShowRegenBuild: false
                    })
                }
  ...
  //重新生成
  onRegenBuild = () => {
    this.setState({isShowRegenBuild: true})
    // this.props.reportFeePayMonthByDate().payload.promise.then((response) => {
    //   let data = response.payload.data;
    //   if (data.state === 'error') {
    //     message.error(data.message);
    //   }
  ...
  render() {
    ...
    switch (this.state.editMode) {
      ...
      default:
        ...
        //超级管理员显示"重新生成"按钮
        if(this.props.currentUser.userType.isAdmin == 1){
          extendButtons.push(<Button onClick={() => { this.onRegenBuild() }} icon="edit" className="button_dark">重新生成</Button>);
        }
        ...
        block_content = (
          <div>
            ...
            {
              this.state.isShowRegenBuild &&
              <Modal
                title="重新生成月报"
                visible={this.state.isShowRegenBuild}
              ...
              >
                <Form>
                  <Row gutter={24}>
                    <Col span={24}>
                        <FormItem {...searchFormItemLayout} label="生成月报日期">
                        {getFieldDecorator('date', {
                                initialValue:dataBind(timestampToTime(this.state.pagingSearch.date), true),
                                rules: [{
                                required: true, message: '请选择生成月报日期!',
                                }],
                            })(
                                <DatePicker
                                format={dateFormat}
...
//表单组件 封装
const WrappedManage = Form.create()(PayfeeStatementManage);
...
function mapDispatchToProps(dispatch) {
  return {
    ...
    selectPayfeeStatement: bindActionCreators(selectPayfeeStatement, dispatch),
    reportFeePayMonthByDate: bindActionCreators(reportFeePayMonthByDate, dispatch),
  };
}
//redux 组件 封装
