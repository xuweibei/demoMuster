...
class CoursePlanView extends React.Component {
      ...
      if(this.state.dataModel.courseplanId){
        this.props.getCoursePlanAuditById(this.state.dataModel.courseplanId).payload.promise.then((response) => {
          ...
          if(data.state === 'success'){
            ...
            this.setState({
              dataModel: data.data,
              isShowFimal: !!data.data.itemStageId,
              isFinal:data.data.isFinal
            }) 
          }
        });
      }
      if(this.props.pagingSearch.itemId){ 
        //获取项目课程阶段列表
        this.props.queryItemStageVosList({itemId:this.props.pagingSearch.itemId,stateMsg:1}).payload.promise.then((response) => {
          let data = response.payload.data; 
          if(data.state === 'success'){
            var list = [];
            data.data.map(item => {
    ...
    renderEditModeOfView() {
      console.log(this.state.isShowFimal)
