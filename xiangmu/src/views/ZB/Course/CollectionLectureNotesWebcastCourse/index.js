...
class StudentInviteManage extends React.Component {
  constructor() {
    ...
    let timeArr2 = new Date();
    let timeArr3 = timestampToTime(timeArr2.setMonth(timeArr[1]-4)).split('-');
    let startTimeYear = timeArr3[0];  
    let startTimeMonth = timeArr3[1];
    let startTimeDay = timeArr3[2];
    this.state = {
      ...
      pagingSearch: {
        ...
        orderSource: '',
        courseName:'',
        activeStartTime:startTimeYear+'-'+startTimeMonth+'-'+startTimeDay,
        activeEndTime:''
      }, 
      data: [],
      totalRecord: 0,
  ...
  columns = [
    ...
    },
    {
      ...
      render: (text, record, index) => {
        if(record.receiveCount==0 || !record.receiveCount) return 0;
        return <a onClick={() => {
          this.onLookView('Number',record)
        }}>{record.receiveCount}</a>;
  ...
  fetch = (params = {}) => {
    ...
    this.props.ManagementOfCollectionList(condition).payload.promise.then((response) => {
      ...
      if (data.result === false) {
        ...
        this.setState({ pagingSearch: condition, ...data, loading: false,courseIdData })
      }
    })
  }
  ...
  render() { 
    ...
    switch (this.state.editMode) { 
      case 'batchImport':
        ...
        break; 
      case 'Number':
        block_content = <DetailView viewCallback={this.onViewCallback} {...this.state}/>
        break;
      case 'Manage':
      default: 
              ...
              {!this.state.seachOptionsCollapsed &&
                ...
                >
                  <Row justify="center" gutter={24} align="middle" type="flex">
                    ...
                    <Col span={12}>
                      <FormItem {...searchFormItemLayout} label={'讲义领取'} >
                    ...
                    <Col span={12} >
                      <FormItem
                        ...
                        label="激活日期"
                      >
                         {getFieldDecorator('activeStartTime', { initialValue: this.state.pagingSearch.activeStartTime?(this.state.pagingSearch.activeEndTime?[moment(this.state.pagingSearch.activeStartTime, dateFormat),moment(this.state.pagingSearch.activeEndTime, dateFormat)]:[moment(this.state.pagingSearch.activeStartTime, dateFormat)]):'',
                         rules: [
                                  { required: true, message: '请选择激活日期!' }
                            ],
                        })(
            ...
            <div className="search-result-list">
              ...
              <div className="search-paging">
                <Row justify="space-between" align="middle" type="flex"> 
                  <Col span={24}>  
                    <Button loading={this.state.loading} icon="database" onClick={this.BatchImport}>批量导入</Button> 
                      <div className='split_button' style={{ width: 10 }}></div>
                      <FileDownloader
                          apiUrl={'/edu/materialOnlineCourseApply/exportMaterialOnlineVos3'}//api下载地址
                          method={'post'}//提交方式
