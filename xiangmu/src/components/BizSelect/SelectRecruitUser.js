...
*/
class SelectRecruitUser extends React.Component {
  constructor(props) {
    super(props)
  ...
  //检索数据
  fetch = (callback) => {
    this.props.postSelectRecruitUser().payload.promise.then((response) => {
      ...
      if (data.result === false) {
        ...
        let data_list = response.payload.data.data.filter((a) => {
          ...
          return { title: item.orgName, value: `${item.orgId}` };
        });
        //动态表达式
        let userDic = {};
...
function mapDispatchToProps(dispatch) {
  return {
    ...
    postSelectRecruitUser: bindActionCreators(postSelectRecruitUser, dispatch),
  };
}
export default connect(mapStateToProps, mapDispatchToProps)(SelectRecruitUser);