...
//业务接口方法引入
import {
  searchUserPartnerScope,//用户负责大客户
  queryPartnerByBranchId
...
class SelectFBOrg extends React.Component {
  ...
  fetch = () => {
      ...
      if (data.result === false) {
        ...
        let all_org_list = data.data.map((item, index) => {
          ...
          return {
            ...
            department: item.department,//分部名称
            partnerClassType: item.partnerClassType//合作方式
          }
        })
        ...
        if (that.state.value) {
          ...
          for (var i = 0; i < data_list.length; i++) {
            if (that.state.value == data_list[i].partnerId) {
    ...
    if (this.props.scope == "branch") {
      this.props.queryPartnerByBranchId(this.props.branchId, this.props.partnerClassType).payload.promise.then((response) => {
        let data = response.payload.data;
        setData(data);
      })
    } else {
      this.props.searchUserPartnerScope({}).payload.promise.then((response) => {
        let data = response.payload.data;
        setData(data);
      })
    }
  ...
  onSelectChange = (value, selectedOptions) => {
    ...
    if (this.props.callback) {
      for (var i = 0; i < this.state.data_list.length; i++) {
        if (this.state.data_list[i].partnerId == value) {
...
function mapDispatchToProps(dispatch) {
  return {
    ...
    searchUserPartnerScope: bindActionCreators(searchUserPartnerScope, dispatch),
