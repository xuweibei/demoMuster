...
class ImportStudentView extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      editMode: '',
  ...
  onExport = () => {
    ...
    var url = `${env.serverURL}/edu/historyCourseArrangeStudent/importStudentExcel?token=${env.getToken()}`;
  ...
  onSave = () => {
    ...
    formData.append('courseplanBatchId',this.props.pagingSearch.courseplanBatchId);
    formData.append('itemId',this.props.pagingSearch.itemId);
    ...
    const successFn = (response) => {
      ...
      if (data.state == 'success') {
        ...
        that.setState({
          ...
          error_url: `/edu/historyCourseArrangeStudent/exportsErrCourse`,
        })
        //param.success({
    ...
    let uploadUrl = `${env.serverURL}/edu/historyCourseArrangeStudent/importStudentExcel?token=${token}`;
    const xhr = new XMLHttpRequest
  ...
  render() {
          ...
          <div>
            <Form className="ant-advanced-search-form" enctype="multipart/form-data" method="post">
              <Row gutter={24}>
                ...
                <Col span={11}>
                  <div style={{ marginBottom: 10,textAlign:'center' }}><FileDownloader
                    apiUrl={'/edu/file/getFile'}//api下载地址
                    method={'post'}//提交方式
                    options={{ filePath: 'upload/importTemplate/importCourseArrangeStudentModel.xlsx' }}
                    title={'下载模板'}
                  >
                  </FileDownloader>
                  </div>
    ...
    return (
      <ContentBox titleName={"导入学生"} bottomButton={
        <FormItem
...
//表单组件 封装
const WrappedManage = Form.create()(ImportStudentView);
