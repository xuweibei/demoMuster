...
import {
  queryByFSubmitProductPricePriceApplyZFinallyExamine, getFinalAuditZ
...
class ProductPriceApplyManage extends React.Component {
  ...
  columns = [
    ...
    },
    {
      ...
      render: (text, record) => (
        <DropDownButton>
          {/* 待终审核或初审未通过且没有新申请情况下可以进入审核操作 */}
          {(record.auditStatus == 1 || (record.auditStatus == 3 && !record.isRepeat)) &&
            <Button onClick={() => { this.onLookView('Audit', record); }}>审核</Button>
          }
          {/* 终审未通过且有新申请情况下或已经进入终审阶段则进入查看操作 */}
          {((record.auditStatus == 3 && record.isRepeat) || record.auditStatus == 2) &&
  ...
  fetch(params = {}) {
    ...
    this.props.queryByFSubmitProductPricePriceApplyZFinallyExamine(condition).payload.promise.then((response) => {
  ...
  onViewCallback = (dataModel) => {
    if (!dataModel) {
      ...
      switch (this.state.editMode) {
        case 'Audit':
          this.props.getFinalAuditZ(dataModel).payload.promise.then((response) => {
            let data = response.payload.data;
            if (data.state === 'error') {
              message.error(data.message);
            }
                      ...
                      <FormItem {...searchFormItemLayout} label="特价审核状态" >
                        {getFieldDecorator('queryAuditState', { initialValue: this.state.pagingSearch.queryAuditState })(
                          <Select>
                            ...
                            {this.state.product_branch_price_status.filter(a => parseInt(a.value) > 2).map((item, index) => {
...
function mapDispatchToProps(dispatch) {
  return {
    ...
    queryByFSubmitProductPricePriceApplyZFinallyExamine: bindActionCreators(queryByFSubmitProductPricePriceApplyZFinallyExamine, dispatch),
    getFinalAuditZ: bindActionCreators(getFinalAuditZ, dispatch),
  };
}
//redux 组件 封装
