...
import { subitem,TransferFinanceList } from '@/actions/course';
import { getBankAccountByZbPayeeType,ShiftWorkStudentPayfeeConfirmBatchConfirm, selectConfirmLastDay } from '@/actions/finance';
import SelectItem from '@/components/BizSelect/SelectItem';

//业务数据视图（增、删、改、查)
import StudentPayfeeConfirmConfirm from './config';
...
class StudentPayfeeConfirmManage extends React.Component {
    ...
    onChangeDate=()=>{
        this.props.form.validateFields((err, values) => {
            ...
            this.props.ShiftWorkStudentPayfeeConfirmBatchConfirm({ studentPayfeeIds: this.state.UserSelecteds.join(','),confirmDate:formatMoment(confir) }).payload.promise.then((response) => {
                let data = response.payload.data;
                if (data.result === false) {
                    message.error(data.message);
                }
                else {
                    message.success('确认成功，成功'+data.data.successCount+'条');
                    this.setState({ UserSelecteds: [],isShowChooseProduct:false })
                    this.onSearch();
                }
            })
        })
    ...
    render() { 
        ...
        switch (this.state.editMode) {
            ...
            case "Confirm":
            case "Edit":
                block_content = <StudentPayfeeConfirmConfirm viewCallback={this.onViewCallback}
                    {...this.state}
                    disabledEndDate={this.disabledEndDate}
                />
                break;
            ...
            default:
                var rowSelection = {
                    ...
                    getCheckboxProps: record => ({
                        disabled: (record.financeStatus == 2), // 暂存或审核未通过的可以提交审核
                    }),
                } 
